...
describe2x2.or <- function(data, outcome=NA, exposure=NA, lang="ro") {
  var_names <- variable.names(data)
  strings <- c()
  
  outcome <- search_for_variable(data, outcome, pattern = NA, type = "binary")
  exposure <- search_for_variable(data, exposure, pattern = NA, type = "binary")
  if(is.na(outcome) || is.na(exposure)) return (NA)
  
  temp <- na.omit(data.frame(`outcome` = data[[outcome]], `exposure` = data[[exposure]]))
  t1 <- xtabs(~outcome+exposure, data=temp)
  t2 <- temp %>% 
    ...
    mutate(
      `Relaive Freq` = Freq/sum(Freq),
      `Freq%` = scales::percent(`Relaive Freq`),
      `sum_outcome` = sum(`Freq`),
      `Label` = paste(`Freq`, " (", `Freq%`, ")", sep="")
    )
  #print (t2)
  
  strings <- c(strings, paste(
    "Dintre cei ", t2$sum_outcome[1], " pacienți cu ", var_names[outcome], ": ", levels(t2$`outcome`)[1], 
    " (", scales::percent(t2$sum_outcome[1]/sum(t2$Freq)), "), ",
    t2$`Label`[1], " au avut ", var_names[exposure], ": ", levels(t2$`exposure`)[1], 
    " față de ", t2$`Label`[3], " dintre cei ", t2$sum_outcome[3], " pacienți cu ", var_names[outcome], ": ", levels(t2$`outcome`)[2], ".",
    sep = ""
  ))
  ...
  #a1 <- DescTools::Assocs(t1)
  a2 <- vcd::assocstats(t1)
  OR = DescTools::OddsRatio(t1, conf.level = 0.95, method = "wald")
  phi  = a2$phi
  ...
  strings <- c(strings, paste(
    "Rezultă că pacienții cu ", var_names[outcome], ": ", levels(t2$`outcome`)[1], " au OR=", 
    ifelse(OR[1]>1, 
           paste(
             scales::number(OR[1], accuracy = 0.1), " [IC95% = ", 
             scales::number(OR[2], accuracy = 0.1), " la ", 
             scales::number(OR[3], accuracy = 0.1), " ] ori mai mare ", 
             sep = ""), 
           paste(
             scales::number(1/OR[1], accuracy = 0.001), " [IC95% = ", 
             scales::number(1/OR[3], accuracy = 0.1), " la ", 
             scales::number(1/OR[2], accuracy = 0.1), " ] ori mai mic ", 
             sep = "")),
    "de a avea ", var_names[exposure], ": ", levels(t2$`exposure`)[1], " decât cei cu ",
    var_names[outcome], ": ", levels(t2$`outcome`)[2], " (", 
    "phi=", scales::number(phi, accuracy = 0.01), ", ", scales::pvalue(a2$chisq_tests[2,3], add_p = T), 
    ifelse(a2$chisq_tests[2,3]<0.05, " semnificativ ", " nesemnificativ "), "statistic conform testului Chi²).",
    sep = ""
  ))
