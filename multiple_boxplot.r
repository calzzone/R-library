...
multiple_boxplot <- function(data, x, y, parse_factors=T) {
  ...
  g <- ggplot(data, aes(x=value,y=y_var))+ theme_gray()+
    ...
    geom_boxplot(fill=colors, outlier.alpha = 0.5, outlier.stroke = 0.5, size=0.5, varwidth = F, coef=Inf)+
...
x = c("Sex", "Mediu", "Fumat", "ADO", "ADI", "Insulina","Retinopatie diabetica", "Neuropatie diabetica")
y = "HbA1c (%)" # "BMI (kg/m²)" 
...
q <- multiple_boxplot(baza_de_date, x, y, parse_factors=F ) +
...
for (v in seq_along(x)) {
  ...
  temp.levels <- baza_de_date[,x[v] ][[1]] %>% na.omit() %>% factor() %>% levels () %>% length()
